...
def get_registration_cars(message):
    ...
    deleter(chat_id, message.id)
    for value in cars:
        if str(cars[value]["master"]) == str(user):
...
def add_wheels(message):
    ...
    bot.send_message(chat_id, '🛞 Введите количество колёс автомобиля', parse_mode='html')
...
def add_brandWs(message):
    ...
    bot.send_message(chat_id, '🔄 Укажите марку шин. Если несколько, введите через запятую', parse_mode='html')
...
def brandWs_property(message):
    ...
    text = message.text
    send_message(chat_id, f'Бренд колес {text} добавлен')
